...
function App(){
  ...
  const load=async()=>{
    ...
    try{
      ...
      if(next.ok===false)throw new Error(next.error||next.message||'Could not load portfolio.');
      setData(next);
  ...
  const refreshQuotes=async()=>{
    ...
    try{
      ...
      const next=await r.json();
      if(!r.ok||next.ok===false)throw new Error(next.error||'Could not refresh prices.');
      setData(d=>({...d,quotes:next.quotes||d.quotes,market:next.market||d.market}));
      await load();
  ...
  return <div className="app">
    ...
    <main className="main">
      ...
      {loading?<Loading/>:<>
        ...
        {tab==='accounts'&&<AccountsPage finance={finance}/>}
...
function Dashboard({totals,holdings,snapshots,market,quotes,finance,period}){
  ...
  return <div className="content">
    ...
    {hasFinance&&safeArray(finance.bankAccounts).length>0&&<div className="dashboard-accounts">{safeArray(finance.bankAccounts).map((a,i)=><div className="dashboard-account" key={a.name||i}><div className="account-icon"><Landmark size={17}/></div><div><span>{a.type||'Bank account'}</span><b>{a.name||'Unnamed account'}</b></div><strong>{money(a.balance)}</strong></div>)}</div>}
...
function AccountsPage({finance}){
  ...
  return <div className="content"><div className="page-intro"><div><span className="eyebrow">CASH</span><h2>Bank accounts</h2><p>Balances come from Finance Assistant. Ledgerly does not maintain a second bank ledger.</p></div><div className="big-total"><span>Total cash</span><b>{money(total)}</b></div></div><div className="account-grid">{accounts.length?accounts.map((a,i)=><div className="account-card" key={a.name||i}><div className="account-icon"><Landmark size={19}/></div><div><b>{a.name}</b><span>{a.type||'Bank account'} · Finance Assistant</span></div><strong>{money(a.balance)}</strong></div>):<Empty title="No bank accounts returned" text="Configure your bank accounts in Finance Assistant." icon={<Landmark size={25}/>}/>}</div><div className="connection-card success"><CheckCircle2 size={21}/><div><b>Finance Assistant is the source of truth</b><p>Expenses, income, transfers and bank balances remain in the Finance Assistant backend. Ledgerly only reads the required data through the private server-to-server bridge.</p></div></div></div>
}
